...
if($val_res_days ==  1){
    ...
    $fecha_ing2 = $row_day['FECHA_INGRESO'];
    ...
    $days_asign_letra = $row_day['DIAS_ASIGN_LETRA'];
    if($days_asign <= 12){
        $per_vac = 1;
        $leyenda_per_vac = " 1 periodo";

    }elseif($days_asign > 12 and $days_asign <= 20  ){
        $per_vac = 2;
        $leyenda_per_vac = " 2 periodos";

    }elseif($days_asign > 20 ){
        $per_vac = 3;
        $leyenda_per_vac = " 3 periodos";
    }

    $min_calendar = date("Y")."-".$mes_ing."-".$dia_ing; // Fecha inicial
    $min_cal_interval = date_create($min_calendar); // se crea objeto de fecha inicial
    $min_calendar2 = date_create($min_calendar); // se crea objeto de fecha inicial
    ...
    if($per_vac == 2){
        date_add($min_cal_interval, date_interval_create_from_date_string('6 months'));
        $fin_interval1 = date_format($min_cal_interval, 'Y-m-d');
        date_add($min_cal_interval, date_interval_create_from_date_string('1 days'));
        $ini_interval2 = date_format($min_cal_interval, 'Y-m-d');
        /*echo "Intervalo 1 {$min_calendar} - {$fin_interval1} <br>
                Intervalo 2 {$ini_interval2} - {$max_calendar} <br>";*/
    }elseif($per_vac == 3){
        date_add($min_cal_interval, date_interval_create_from_date_string('4 months'));
        $fin_interval1 = date_format($min_cal_interval, 'Y-m-d');
        date_add($min_cal_interval, date_interval_create_from_date_string('1 days'));
        $ini_interval2 = date_format($min_cal_interval, 'Y-m-d');
        date_add($min_cal_interval, date_interval_create_from_date_string('4 months'));
        $fin_interval2 = date_format($min_cal_interval, 'Y-m-d');
        date_add($min_cal_interval, date_interval_create_from_date_string('1 days'));
        $ini_interval3 = date_format($min_cal_interval, 'Y-m-d');
            /*echo "Intervalo 1 {$min_calendar} - {$fin_interval1} <br>
                Intervalo 2 {$ini_interval2} - {$fin_interval2} <br>
                Intervalo 2 {$ini_interval3} - {$max_calendar} <br>";*/
    }

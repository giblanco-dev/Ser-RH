...
require_once '../../logic/conn.php';
    ...
    $sql_sol_del = "SELECT id_solicitud, fecha_ini, fecha_fin, id_empleado FROM sol_vacaciones where id_solicitud = $solicitud and estatus = 0;";
    ...
    if($mysqli->query($sql_upd_calendar) === True){
            ...
            $sql_del_sol = "DELETE FROM sol_vacaciones where id_solicitud = $solicitud and estatus = 0;";

                if($mysqli->query($sql_del_sol) === True){
                        $title = "La solicitud {$solicitud} fue liberada";
                        $text = "Validar días pendientes";
                        $icon = "success";
                }
...
<head>
    ...
    <title>Liberación Solicitud de Vacaciones</title>
